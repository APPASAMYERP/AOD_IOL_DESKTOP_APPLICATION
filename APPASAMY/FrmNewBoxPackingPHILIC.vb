...
Public Class FrmNewBoxPackingPHILIC
    ...
    Private Sub FrmNewBoxPackingPHILIC_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        killProcess("bartend")
    End Sub
    ...
    Private Sub BatchNo_Load()
        ...
        End If
    End Sub
    Private Sub FrmNewBoxPackingPHILIC_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If txtlotbarno.Text <> "" Then
                ...
                Try
                    ...
                    If ckReprint.Checked = False Then
                        If cmbBPL.Text = "" Then
                            MsgBox("Please select BPL ")
                            txtlotbarno.Text = ""
                            txtlotbarno.Focus()
                            Exit Sub
                        End If
                    ...
                    If txtlotbarno.Text <> "" Then
                        ...
                        If rbOthers.Checked Then
                            ...
                            ElseIf rdInjLot.Checked = True Then
                                StrSelInj = "SELECT @Lot_srno VARCHAR(50)  = '" & txtinj_No.Text & "' SELECT Inj_ref,Str_batch,Exp_year,Exp_Month,Mfd_Year,Mfd_Month,Lot_srno from Injector_Label where Lot_srno = @Lot_srno "
                            Else
                                MsgBox(" Plese choose Injector Batch or Injector Lot")
                                Exit Sub
                            End If
                        ...
                        If ChkCST.Checked = True Then
                            ...
                            Else
                                If rdLotSerial.Checked = True Then
                                    StrSeSql = "DECLARE @Lot_SrNo NVARCHAR(50) = '" & txtlotbarno.Text & "'  SELECT Brand_Name,Model_Name,Reference_Name,GS_Code,Power,Optic,Length,A_Constant,Type_GS_Code,Lot_Prefix,Lot_No,Lot_SrNo,Mfd_Month,Mfd_Year,Exp_Month,Exp_Year,Type,Btc_No,price,R_Power,Injector_Ref,Injector_batch, Print_Name, Box_Packing from temp_POUCH_LABEL_box where Lot_SrNo=@Lot_SrNo " & _
                               "and Sterilization=1  and Sample_Taken=1 and Type_Sample='CST' and Dc_Packing=0  and Box_Reject = 0 and Sterilization_After = 1 and Sterilization_Reject = 0  and  Btc_No is not null  "
                                ElseIf rdUDICode.Checked = True Then
                                    StrSeSql = "DECLARE @Lot_SrNo NVARCHAR(100) = '" & txtlotbarno.Text & "'  SELECT Brand_Name,Model_Name,Reference_Name,GS_Code,Power,Optic,Length,A_Constant,Type_GS_Code,Lot_Prefix,Lot_No,Lot_SrNo,Mfd_Month,Mfd_Year,Exp_Month,Exp_Year,Type,Btc_No,price,R_Power,Injector_Ref,Injector_batch, Print_Name, Box_Packing from temp_POUCH_LABEL_box where Udi_code=@Lot_SrNo " & _
                               "and Sterilization=1  and Sample_Taken=1 and Type_Sample='CST' and Dc_Packing=0  and Box_Reject = 0 and Sterilization_After = 1 and Sterilization_Reject = 0   and  Btc_No is not null  "
                                Else
                                    MsgBox(" Plese choose Lot Serial or UDI Serial")
                                    Exit Sub
                        ...
                        If StrSeRs.Read Then
                            ...
                            strPrintName = IIf(IsDBNull(StrSeRs.GetValue(22)), 0, StrSeRs.GetValue(22))
                                ...
                                If DataGridView2.Item(0, i).Value.ToString() = StrLotBarNo Then
                                    If DataGridView2.Item(4, i).Value.ToString() <> "1" Then
                                        ...
                                        If rdLotSerial.Checked = True Then
                                            ...
                                            'SqlIn = "update POUCH_LABEL set Box_Packing =1,Box_By='" & StrLoginUser & "',Injector_Ref ='" & strinj_ref & "',Injector_batch ='" & strinj_batch & "',Box_Date=GETDATE(),Boxtime = GETDATE(),Station = '" & station & "',Injector_Lot = '" & strinj_Lot & "',BoxBTWLabelName = '" & cmbPrintLabel.Text & "' where Lot_SrNo='" & txtlotbarno.Text & "' "
                        ...
                        bt.SetNamedSubStringValue("opt_Len", Strsize)
                        ...
                        'Dim pdfFilePath As String = Application.StartupPath & "\" & station & "\" & DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss") & "-" & StrLotBarNo & "-" & StrFName & ""
                        '' Ensure directory exists
                        'Dim directoryPath As String = System.IO.Path.GetDirectoryName(pdfFilePath)
    ...
    Private Sub rbheerafold_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbheerafold.CheckedChanged
    ...
    Private Sub BtnStart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnStart.Click
        Try
            ...
            "  Exp_Year, Sterilization, Sterilization_No, Sample_Taken, Type_Sample, BPL_Taken, BPL_NO, Box_Packing, Dc_Packing, Dc_No, Sterilization_After, Sterilization_Reject, Qty_1, Type, Box_Reject, " & _
            " Box_Reject_Date, Box_Reject_By, Box_By, Box_Date, Print_Name, Packing_Model, Ord_Country, Btc_No, price, Sterility_Status, Injector_Ref, Injector_batch, R_Power, Cylsize, Label, RefLot, " & _
            " Status, Boxtime, Station, Udi_code, Injector_Lot, BPL_Collection_Status, pouchRepack_staus, bpl_cancel_status, BPL_Collected_by, Areation_Status, BoxBTWLabelName " & _
            "FROM POUCH_LABEL " & _
            ...
            If lblpackedQty.Text = lblTotalQty.Text Then
                ...
                Try
                    ...
                    If ChkCST.Checked = True Then
                        ...
                        'update Boxtime ,Injector_Ref , Injector_batch, BoxBTWLabelName
