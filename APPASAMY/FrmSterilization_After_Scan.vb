...
Public Class FrmSterilization_After_Scan
    ...
    Private Sub Form_Load()
        ...
        StrSql = "Select Distinct Btc_no from Pouch_Label where (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1') AND  (udi_code is not NULL)  and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1 AND (Box_Packing = '0') AND (Areation_Status is NULL) and Btc_No not in(" & SterileBatches & ") "
    ...
    Public Function getLotSerialNumber() As DataSet
        ...
        "where Btc_no = '" & cmbBatch.Text & "'  AND (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1') AND  (udi_code is not NULL)  and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1  AND (Box_Packing = '0')  " & _
        " order by  Lot_SrNo"
        Dim Cmd As New SqlCommand(StrSql, con)
        Dim ad As New SqlDataAdapter(Cmd)
        ad.Fill(ds)
    ...
    Private Sub to_create_records()
        StrSql = " INSERT INTO temp_POUCH_LABEL_Areation " & _
         ...
         "where Btc_no = '" & cmbBatch.Text & "'  AND (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1') AND  (udi_code is not NULL)  and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1 AND (Box_Packing = '0') AND  " & _
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If txtLotSrNo.Text <> "" Then
                ...
                Try
                    ...
                    Sql = "select Model_Name,Btc_no,Power,Lot_srno from temp_POUCH_LABEL_Areation where Udi_Code='" & txtLotSrNo.Text & "' AND Btc_no = '" & cmbBatch.Text & "' AND (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1')   and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1 AND (Box_Packing = '0') AND pouchRepack_staus is NULL order by lot_srno "

                    If rdLotSerial.Checked = True Then
                        Sql = "select Model_Name,Btc_no,Power,Lot_srno from temp_POUCH_LABEL_Areation where Lot_Srno='" & txtLotSrNo.Text & "' AND Btc_no = '" & cmbBatch.Text & "' AND (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1')    and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1 AND (Box_Packing = '0') AND pouchRepack_staus is NULL order by lot_srno "

                    ElseIf rdUDICode.Checked = True Then
                        Sql = "select Model_Name,Btc_no,Power,Lot_srno from temp_POUCH_LABEL_Areation where Udi_Code='" & txtLotSrNo.Text & "' AND Btc_no = '" & cmbBatch.Text & "' AND (BPL_Taken = '0') AND (Dc_No IS NULL) AND (Sterilization_After = '1') AND (Sterilization_Reject = '0') AND (Sample_Taken = '0') AND (Sterilization = '1')   and Dc_Packing='0'  AND Sterile_to_Areation_Move_status = 1 AND (Box_Packing = '0') AND pouchRepack_staus is NULL order by lot_srno "

                    Else
                        Dim customMsgBox As New CustomMessageBox("Please Select 'Lot Serial' Or 'UDI Serial'")
    ...
    Private Sub btnComplete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnComplete.Click
        ...
        Try
            ...
            If lblScannedQty.Text = lblTotalQty.Text Then
                ...
                Sql = "Insert Into Areation_Scan_Details ( Header_ID, Detail_ID, Areation_Scan_No, Created_By, Created_Date, Modified_By, Modified_Date, Btc_No,  Qty ) values ( " & _
                                  "'" & Str_Header & "','" & Str_Detail & "', '" & lblAreationNo.Text & "', " & _
                                  "'" & StrLoginUser & "',GETDATE(), '" & StrLoginUser & "',GETDATE(),'" & cmbBatch.Text & "' ,'" & Val(lblScannedQty.Text) & "' )"
                ...
                Sql = "DELETE FROM temp_POUCH_LABEL_Areation WHERE     (Btc_No = '" & cmbBatch.Text & "' ) "
                UpdateorInsertQuery_Execute(Sql)
