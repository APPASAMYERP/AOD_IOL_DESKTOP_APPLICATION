...
Public Class frmBeforeSterilization
    ...
    Private Sub btnComplete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnComplete.Click
        ...
        Try
            ...
            If lblScannedQty.Text = lblTotalQty.Text Then
                ...
                Sql = "DECLARE @Btc_No  @nvarchar(50)='" & cmbBatch.Text & "' DELETE FROM temp_POUCH_LABEL WHERE     (Btc_No = @Btc_No ) "
                UpdateorInsertQuery_Execute(Sql)
    ...
    Public Function check_btc_data_created() As Boolean
        ...
        Dim ds As New DataSet
        Dim StrSql As String = "Declare @Btc_no nvarchar(50)= '" & cmbBatch.Text & "' SELECT Count(*) As Total from temp_Pouch_Label  " & _
        "where Btc_no = @Btc_no AND  (Btc_No IS NOT NULL) AND (Sterilization_After = '0') AND (Sample_Taken = '0') AND (Sterilization = '0') AND (Dc_Packing = '0') AND (Dc_No IS NULL) AND (BPL_Taken = '0')  "
        Dim Cmd As New SqlCommand(StrSql, con)
        Dim ad As New SqlDataAdapter(Cmd)
        ad.Fill(ds)
    ...
    Private Sub to_create_records()
        StrSql = "Declare @Btc_no nvarchar(50)= '" & cmbBatch.Text & "' INSERT INTO temp_Pouch_Label " & _
            "SELECT * from Pouch_Label   " & _
         "where Btc_no = @Btc_no AND  (Btc_No IS NOT NULL) AND (Sterilization_After = '0') AND (Sample_Taken = '0') AND (Sterilization = '0') AND (Dc_Packing = '0') AND (Dc_No IS NULL) AND (BPL_Taken = '0') AND  " & _
    ...
    Public Function getLotSerialNumber() As DataSet
        ...
        Dim ds As New DataSet
        Dim StrSql As String = "Declare @Btc_no nvarchar(50)= '" & cmbBatch.Text & "' SELECT DISTINCT Lot_SrNo, Model_Name, Power,Btc_No, Sterilization_scan from temp_Pouch_Label  " & _
        "where Btc_no = @Btc_no AND  (Btc_No IS NOT NULL) AND (Sterilization_After = '0') AND (Sample_Taken = '0') AND (Sterilization = '0') AND (Dc_Packing = '0') AND (Dc_No IS NULL) AND (BPL_Taken = '0')  " & _
        " order by  Lot_SrNo"
        Dim Cmd As New SqlCommand(StrSql, con)
        Dim ad As New SqlDataAdapter(Cmd)
        ad.Fill(ds)
