...
Public Class frmShrinkWarp_Rejection_MMS_Report
    ...
    Public Sub Form_Load()
        Try
            StrSql = " SELECT     Distinct Movement_No " & _
                            " FROM          Rejection_MTS_Details_ShrinkWarp   order by  Movement_No DESC "
    ...
    Private Sub frmShrinkWarp_Rejection_MMS_Report_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Form_Load()
    End Sub
        ...
        Try
            ...
            If CmbMTSNo.Text <> "" Then
                ...
                StrSql = " SELECT     distinct  Location_from, Location_To " & _
                        " FROM        Rejection_MTS_Details_ShrinkWarp  WHERE    Movement_No = '" & CmbMTSNo.Text & "'   "
                Ds = SQL_SelectQuery_Execute(StrSql)
                form_loc = Ds.Tables(0).Rows(0)("Location_from")
                to_loc = Ds.Tables(0).Rows(0)("Location_To")

                StrSql = " SELECT     Brand_Name, Model_Name, Power, Lot_SrNo, Qty_1 " & _
                        " FROM         POUCH_LABEL " & _
                        "WHERE     (ShrinkWarp_Rejection_Move_No = '" & CmbMTSNo.Text & "') " & _
