...
Public Class FrmNewCtrlSample
    ...
    Dim readcheck As SqlDataReader
    ...
    Private Sub FrmNewSterSample_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Str_Header = 0
        StrSqlSeHd = "Select Max(Header_ID) from CONTROL_SAMPLE"
        ...
        StrUniqueNo = "CST/" & Format(Now, "ddMMyy") & "/" & Str_Header
    ...
    Private Sub BtnComplete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnComplete.Click
        ...
        Try
            ...
            Str_Detail = 0
            StrSqlSeDt = "Select Max(Detail_ID) from CONTROL_SAMPLE"
            ...
            Sql = "INSERT Into CONTROL_SAMPLE  (Header_ID,	Detail_ID,	Lot_SrNo,Ct_Test_No,	Created_By,	Created_Date,	Modified_By,	Modified_Date )VALUES " & Sql
            cmd = New SqlCommand(Sql, con)
            cmd.ExecuteNonQuery()
            cmd.Dispose()

            Sql = strUpdateQry.Remove(strUpdateQry.Length - 1, 1)
            Sql = "Update POUCH_LABEL set Sample_Taken='1',type_Sample='CST' where Lot_SrNo IN (" & Sql & ")"
            cmd = New SqlCommand(Sql, con)
            cmd.ExecuteNonQuery()
            cmd.Dispose()
            ...
            If txtlotbarno.Text <> "" Then
                ...
                Try
                    ...
                    If rdLotSerial.Checked = True Then
                        Sql = "select Lot_Prefix,Lot_No,Lot_SrNo,btc_no from POUCH_LABEL where Lot_SrNo='" & txtlotbarno.Text & "' and " & _
                 "Sterilization=1 and Type_Sample <>'SST' and Sample_Taken =0 and BPL_Taken=0 and Box_Packing=0 and " & _
                 "Dc_Packing=0 AND Sterilization_After=1 AND Sterilization_Reject = 0"
                    ElseIf rdUDICode.Checked = True Then
                        Sql = "select Lot_Prefix,Lot_No,Lot_SrNo,btc_no from POUCH_LABEL where Udi_code='" & txtlotbarno.Text & "' and " & _
                 "Sterilization=1 and Type_Sample <>'SST' and Sample_Taken =0 and BPL_Taken=0 and Box_Packing=0 and " & _
                 "Dc_Packing=0 AND Sterilization_After=1 AND Sterilization_Reject = 0"
